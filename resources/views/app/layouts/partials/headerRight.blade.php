...
>
    ...
    <div class="navigation-dropdown md:hidden md:opacity-0">
        <a x-on:click="select" class="navigation-link" href="{{ route('account') }}">{{ __mc('Profile') }}</a>
        <a x-on:click="select" class="navigation-link" href="{{ route('general-settings') }}">{{ __mc('Configuration') }}</a>
        <form class="navigation-link" method="post" action="{{ route('mailcoach.logout') }}">
            {{ csrf_field() }}
            <button type="submit" class="font-semibold">
                <x-mailcoach::icon-label invers caution icon="fas fa-fw fa-power-off" :text="__mc('Log out')" />
            </button>
        </form>
    </div>
</div>
